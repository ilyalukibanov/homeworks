...
def regularized_linear_regression(X, y, lambd):
  ...
  #####################################################
  #				 YOUR CODE HERE					                    #
  w = np.linalg.inv(X.transpose() @ X + lambd * np.identity(X.shape[1])) @ X.transpose() @ y
  #####################################################		 
  return w
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
  ...
  #####################################################
  #				 YOUR CODE HERE					                    #
  mse_best = test_error(regularized_linear_regression(Xtrain, ytrain, lambds[0]), Xval, yval)
  bestlambda = lambds[0]
  for lambd in lambds:
    current_mse = test_error(regularized_linear_regression(Xtrain, ytrain, lambd), Xval, yval)
    if current_mse < mse_best:
      mse_best = current_mse
      bestlambda = lambd
  #####################################################		 
  return bestlambda
...
def test_error(w, X, y):
  """
    ...
    """
  err = (X @ w - y).transpose() @ (X @ w - y) / X.shape[0]
  return err
