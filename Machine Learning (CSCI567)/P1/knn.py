...
def compute_distances(Xtrain, X):
	...
	for i in range(X.shape[0]):
		for j in range(Xtrain.shape[0]):
			dists[i,j] = np.sqrt((X[i,:] - Xtrain[j,:]).transpose() @ (X[i,:] - Xtrain[j,:]))
	#####################################################
	return dists
...
def predict_labels(k, ytrain, dists):
	...
	#####################################################
	#				 YOUR CODE HERE					                    #
	ypred = np.ones((dists.shape[0],)) * np.amin(ytrain)
	for i in range(dists.shape[0]):
		knn = ytrain[np.argsort(dists[i,:])[0:k]]
		max_in_one_class = np.count_nonzero(knn == np.amin(knn))
		ypred[i] = np.amin(knn)
		for unn in np.unique(knn):
			current_in_class = np.count_nonzero(knn == unn)
			if current_in_class > max_in_one_class:
				ypred[i] = unn
				max_in_one_class = current_in_class
	#####################################################
	return ypred
...
def compute_accuracy(y, ypred):
	...
	#####################################################
	#				 YOUR CODE HERE					                    #
	acc = np.count_nonzero(y == ypred)/y.shape[0]
	#####################################################
	return acc
...
def find_best_k(K, ytrain, dists, yval):
	...
	#				 YOUR CODE HERE					                    #
	best_k = np.amin(K)
	best_accuracy = compute_accuracy(yval, predict_labels(best_k,ytrain, dists))
	validation_accuracy = []

	for k in K:
		current_accuracy = compute_accuracy(yval, predict_labels(k,ytrain, dists))
		if current_accuracy > best_accuracy:
			best_k = k
			best_accuracy = current_accuracy
		validation_accuracy = np.append(validation_accuracy,current_accuracy)
	#####################################################
