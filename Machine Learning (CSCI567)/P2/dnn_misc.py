...
class linear_layer:
    ...
    def forward(self, X):
        ...
        ################################################################################
        forward_output = X @ self.params['W'] + np.tile(self.params['b'],(X.shape[0],1))
        #print("linear_layer_forward")
    ...
    def backward(self, X, grad):
        ...
        self.gradient['b'] = np.ones(X.shape[0]) @ grad
        backward_output = grad @ self.params['W'].T
        #print("linear_layer_back")
...
class relu:
    ...
    def forward(self, X):
        ...
        ################################################################################
        forward_output = np.clip(X, 0, np.Inf)
        #print(X.shape)
        #print("relu_forw")
        #print(forward_output.shape)
    ...
    def backward(self, X, grad):
        ...
        # PLEASE follow the Heaviside step function defined in CSCI567_HW2.pdf                                                   #
        ##########################################################################################################################
        backward_output = np.multiply((X > 0), grad)
        #print("relu_back")
        #print(backward_output.shape)
