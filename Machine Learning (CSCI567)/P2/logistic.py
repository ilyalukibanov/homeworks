...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    current_iteration = 0
    ones = np.ones(N)
    X_tilda = X - np.repeat((np.amax(X, axis=1)).reshape((N, 1)), D, axis=1)
    while not_converged:
        w_old = w
        b_old = b
        w += step_size * (X_tilda.transpose() @ (y - sigmoid(X_tilda @ w + ones.transpose() * b))) / N
        b += step_size * (ones @ (y - sigmoid(X_tilda @ w + ones.transpose() * b))) / N
        current_iteration += 1
        if current_iteration >= 1000| (((w - w_old).transpose() @ (w - w_old) + (b - b_old)*(b - b_old)) < 0.00000001):
            not_converged = False
...
def multinomial_train(X, y, C, 
    ...
    for i in range(0, max_iterations):
        param = X_tilda @ w.T
        param -= np.amax(param, axis = 1, keepdims = True)
        current_exp_sum =  np.sum(np.exp(param), axis = 1)
        for c in range(1,C):
            prob = np.exp(param[:,c]) / current_exp_sum
            w[c, :] += step_size * (X_tilda.transpose() @ ((y == c) - prob)) / N
            b[c] += step_size * (ones @ ((y == c) - prob)) / N
...
def multinomial_predict(X, w, b):
    ...
    prob = np.zeros((N, C))
    for c in range(0, C):
        prob[:,c] = X @ w[c,:].transpose() + b[c] * np.ones(N).transpose()
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for c in range(0, C):
        y_c = (y == c).astype(int)
        w[c,:], b[c] = binary_train(X, y_c, w0=None, b0=None, step_size=step_size, max_iterations=max_iterations)
...
def OVR_predict(X, w, b):
    ...
    for c in range(0, C):
        probabilities[:,c] = sigmoid(X @ w[c,:].transpose() + b[c]*np.ones(N).transpose())
