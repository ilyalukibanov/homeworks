...
def viterbi(pi, A, B, O):
  ...
  for i in range(1, N):
      arg = (A.T @ d[:, i-1]).T
      d[:, i] = B[:, O[i]] * np.amax(arg, 0)
      D[:, i] = np.argmax(arg, 0)
  path.insert(0, int(np.argmax(d[:, N-1])))
  for i in range(N-2, -1, -1):
      path.insert(0, int(D[path[0], i+1]))
  return path
