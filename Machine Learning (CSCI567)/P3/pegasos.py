...
def objective_function(X, y, w, lamb):
    ...
    a = (1- y * (X @ w))
    obj_value = lamb/2 * w.T @ w + np.ones(a[a>=0].shape[0]).T @ a[a>=0]/X.shape[0]
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        X = np.asarray(Xtrain)
        y = np.asarray(ytrain)
        A_plus = A_t[ y[A_t] * np.reshape((X[A_t,:] @ w),(k,)) < 1]
        eta_t = 1 / (lamb * iter)
        w_t_half = np.reshape((1-eta_t*lamb) * w,(w.shape[0],)) + eta_t/k * y[A_plus].T @ X[A_plus,:]
        w = np.amin([1, 1/np.sqrt(lamb)/np.sqrt(w_t_half.T @ w_t_half)])*w_t_half
...
def pegasos_test(Xtest, ytest, w_l):
    ...
    """
    # you need to fill in your solution here
    test_acc = (np.asarray(ytest) * (np.asarray(Xtest) @ w_l) > 0).sum()/np.asarray(ytest).shape[0]
